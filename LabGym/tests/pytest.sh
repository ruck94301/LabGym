# This script
#  *  provides add'l config for standard pytest
#  *  expects to be run in a venv with pytest and pytest-cov
...
#     sh pytest.sh  # same as "sh pytest.sh ."
#
#     # Run the tests in this dir, except those marked with wxapp marker.
#     sh pytest.sh -m "not marker_wxapp"
#
#     # Run the tests in this dir, change the delay
#     sh pytest.sh --multiplier=8.0
#
#     # Run the tests in this dir, except ignore tmp* during collection.
#     sh pytest.sh --ignore-glob="tmp*"
#
#     # Run the tests in ./test_myargparse.py.
#     sh pytest.sh test_myargparse.py
#
#     # Run one specific test in ./test_myargparse.py.
#     sh pytest.sh test_myargparse.py::test_parse_args_no_args
#
#     # This produces an error, because zsh finds and tries to run 
#     # .../bin/pytest, which is a python script, not an sh script.
#     sh pytest  # (missing .sh suffix)
#
# Examples of running pytest without this wrapper script
#     # run default pytest on .
#     pytest . --ignore-glob="tmp*"
#
#     # run default pytest on .. 
#     pytest .. --ignore-glob="../tmp*" --ignore-glob="../tests/tmp*"
#     # Why doesn't --ignore-glob="**/tmp*" work ?!  
#     # Investigate with --collect-only, --co  Only collect tests, don't execute them
...
printf "See coverage report at file://$PWD/htmlcov/index.html\n"
